...
def getPRNG(a,b,c,x0,y0,p1,z0,p2):
    ...
    SortLen = (a//2)*(b//2)*c # if the a and c are not the interal multiple of 2, discard the last row and column 
...
def calc(img,itype):
    ...
    np.random.seed(1111)
    position = np.random.randint(254,size=(5000,2))
    #print(position)
...
def globalEntropyTest():
    ...
    for picName in pictures:
        picPath = path + picName
        print(picName,"'s Global entropy:")
        lenna_img = np.array(Image.open(picPath).convert('L'))
        lenna_cipher = DoEnc(lenna_img,key)
        lenna_img = np.array(Image.open(picPath).convert('L'))
...
def LocalEntropy(cipher):
    ...
    print("average local entropy:", averagenum(localEntropy))
    return

def localEntropyTest():    
    path = "./512x512/"
    pictures = os.listdir(path)
    key = [0x7833A013F4DB0018F4FB4031E9F680BC614A,0x7833A013F4DB0018F4FB4031E9F680BC614B] 
    icount = 0
    for picName in pictures:
        picPath = path + picName
        print(picName,"'s Local Shannon entropy:")
        lenna_img = np.array(Image.open(picPath).convert('L'))
        lenna_cipher = DoEnc(lenna_img,key)
        LocalEntropy(lenna_cipher)
    return 0
...
# In[50]:
#CASE: when the with and hight of image are not the integral multiple of 4

def Multiple4Process(imgPath):
    #process the image to make the w and h are not the integral multiple of 4
    print("----------------")
    ...
    if h%4 == 0:
        h = h-1
    resImg = img[0:w,0:h]
    print("the oral img's shape is :" , img.shape)
    print("the output img's shape is : " , resImg.shape)
    return resImg

def Multiple4Enc(img):
    key = [0x7833A013F4DB0018F4FB4031E9F680BC614A,0x7833A013F4DB0018F4FB4031E9F680BC614B]
    plt.figure(figsize=(26,25),dpi=80)
    plt.rcParams['axes.titlesize'] = 25
    plt.rcParams['xtick.labelsize'] = 20
    plt.rcParams['ytick.labelsize'] = 20
    
    plt.subplot(1,3,1),plt.imshow(img,cmap="gray")
    plt.title("255x255 image")
    
    time_start=time.time()
    img_cipher = DoEnc(img,key)
    time_end=time.time()    
    plt.subplot(1,3,2),plt.imshow(img_cipher,cmap="gray")
    plt.title("Encrypted")


    time_DecStart=time.time()
    img_dec = DoDec(img_cipher,key)
    time_DecEnd=time.time()
    
    plt.subplot(1,3,3),plt.imshow(img_dec,cmap="gray")
    plt.title("Decrypted")
...
def testMultiple4():
    key = [0x7833A013F4DB0018F4FB4031E9F680BC614A,0x7833A013F4DB0018F4FB4031E9F680BC614B]
    #NPCR AND UACI
    testImg = Multiple4Process("./Multiple4Test/5.1.12.tiff")
    lenna_cipher = DoEnc(testImg,key)

    testImg = Multiple4Process("./Multiple4Test/5.1.12.tiff")
    lenna_img2 = GetSlightImg(testImg)
    lenna_cipher2 = DoEnc(lenna_img2,key)
    
    NPCRvalue = NPCR(lenna_cipher,lenna_cipher2)

    UACIvalue = UACI(lenna_cipher,lenna_cipher2,255)
    print("NPCR value is:", NPCRvalue)
    print("UACI value is:", UACIvalue)
    
    #Histogram Variance
    arr=lenna_cipher.flatten() 
    varValue = Variance(arr,arr.size)
    print("Histogram Variance value is:", varValue)
...
def oddSizeTest():
    testImg = Multiple4Process("./Multiple4Test/5.1.12.tiff")
    Multiple4Enc(testImg)
    testMultiple4()
    return 0

oddSizeTest()
print("proposed algorithm psnr: 38.823491357")
